...
void PidTuner::tuning_loop() {
  ...
  if (myMachine->inputTemp < (myMachine->myConfig->targetTemp -
                              TuneThres)) { // below lower threshold -> power on
    if (myMachine->outputPwr == 0) {        // just fell below threshold
      if (tune_count == 0)
        tune_start = time_now;
      tune_time = time_now;
      tune_count++;
  ...
  } else if (myMachine->inputTemp >
             (myMachine->myConfig->targetTemp +
              TuneThres)) { // above upper threshold -> power off
    if (myMachine->outputPwr == TuneStep) { // just crossed upper threshold
...
unsigned long PidTuner::timeElapsed() { return (abs(tune_time - tune_start)); }
...
void PidTuner::setTuneCount(int i) { tune_count = i; }
int PidTuner::getTuneCount() { return (tune_count); }

void PidTuner::setTuneStep(double i) { TuneStep = i; }
void PidTuner::setTuneThres(double i) { TuneThres = i; }
double PidTuner::getTuneStep() { return (TuneStep); }
double PidTuner::getTuneThres() { return (TuneThres); }
