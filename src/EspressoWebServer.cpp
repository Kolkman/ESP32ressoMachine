...
EspressoWebServer::EspressoWebServer(uint16_t port, const char *username,
                                     const char *password)
    : AsyncWebServer(port) {
...
char *EspressoWebServer::getUsername() { return username; }

char *EspressoWebServer::getPassword() { return password; }
...
void EspressoWebServer::authenticate(AsyncWebServerRequest *request) {
  ...
  if (authRequired) {
    ...
    response->addHeader(
        "Location",
        "/WebLogin.html?msg=Authentication Failed, you must log in.");
...
// from
// https://www.mischianti.org/2020/11/09/web-server-with-esp8266-and-esp32-manage-security-and-authentication-4/#Simple_token_authentication
...
void EspressoWebServer::handleLogin(AsyncWebServerRequest *request) {
  ...
  if (request->hasArg("username") && request->hasArg("password")) {
    ...
    if (request->arg("username") == String(username) &&
        request->arg("password") == String(password)) {
      AsyncWebServerResponse *response =
          request->beginResponse(301); // Sends 301 redirect
      ...
      response->addHeader("Cache-Control", "no-cache");
      String token = sha1(String(username) + ":" + String(password) + ":" +
...
void EspressoWebServer::InitPages() {
  this->onNotFound(std::bind(&EspressoWebServer::handleNotFound, this,
                             std::placeholders::_1));
  this->on(
      "/login", HTTP_POST,
      std::bind(&EspressoWebServer::handleLogin, this, std::placeholders::_1));
  this->on(
      "/logout", HTTP_GET,
      std::bind(&EspressoWebServer::handleLogout, this, std::placeholders::_1));
...
void EspressoWebServer::handleFile(AsyncWebServerRequest *request,
                                   const char *mimetype,
