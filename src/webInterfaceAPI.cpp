...
void webInterfaceAPI::begin(EspressoWebServer *s, ESPressoMachine *m) {
  ...
  server->on(
      ...
      std::bind(&webInterfaceAPI::handleStatus, this, std::placeholders::_1));
  server->on(
      "/api/v1/firmware", HTTP_GET,
      std::bind(&webInterfaceAPI::handleFirmware, this, std::placeholders::_1));
  server->on(
      "/api/v1/get", HTTP_GET,
      std::bind(&webInterfaceAPI::handleGet, this, std::placeholders::_1));
  server->on(
      "/api/v1/set", HTTP_GET,
      std::bind(&webInterfaceAPI::handleSet, this, std::placeholders::_1));
  server->on(
      "/api/v1/pwr", HTTP_GET,
      std::bind(&webInterfaceAPI::handlePwr, this, std::placeholders::_1));
  server->on(
      "/api/v1/statistics", HTTP_GET,
      std::bind(&webInterfaceAPI::handleStats, this, std::placeholders::_1));
...
void webInterfaceAPI::handleFirmware(AsyncWebServerRequest *request) {
  String message = "{\"version\": \"" + String(CURRENTFIRMWARE) + "-" +
...
void webInterfaceAPI::handleGet(AsyncWebServerRequest *request) {
  ...
  for (uint8_t i = 0; i < request->args(); i++) {
    if (request->argName(i) == "tset") {
      ...
      addjson(message, firstarg, "eqPwr", String(myMachine->myConfig->eqPwr));
...
void webInterfaceAPI::handleSet(AsyncWebServerRequest *request) {
  ...
  for (int z = 0; z < NUM_WIFI_CREDENTIALS; z++) {
    myMachine->myConfig->WM_config.WiFi_Creds[z].config_change =
        false; // initiate
  }
  for (uint8_t i = 0; i < request->args(); i++) {
    if (request->argName(i) == "tset") {
      ...
      if ((request->arg(i)).toDouble() > 110) { // failsafe
      ...
      for (int z = 0; z < NUM_WIFI_CREDENTIALS; z++) {
        ...
        if (request->argName(i).equals("wifi_ssid" + String(z)) &&
          ...
          strlcpy(myMachine->myConfig->WM_config.WiFi_Creds[z].wifi_ssid,
      ...
      for (int z = 0; z < NUM_WIFI_CREDENTIALS; z++) {
        ...
        if (request->argName(i).equals("wifi_pw" + String(z)) &&
          ...
          strlcpy(myMachine->myConfig->WM_config.WiFi_Creds[z].wifi_pw,
      ...
      strlcpy(myMachine->myConfig->mqttUser, request->arg(i).c_str(),
              MQTTUSERFIELDS_LENGTH);
    } else if (request->argName(i) == "mqttPass") {

      strlcpy(myMachine->myConfig->mqttPass, request->arg(i).c_str(),
              MQTTUSERFIELDS_LENGTH);
