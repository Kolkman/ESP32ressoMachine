...
{ // only used as config portal
  ...
  AsyncWebServerResponse *response = request->beginResponse(200, "text/html;charset=UTF-8", networkSetup_html, networkSetup_html_len);
...
void WebInterface::handleCaptivePortal(AsyncWebServerRequest *request)
{
  ...
  AsyncWebServerResponse *response = request->beginResponse(200, "text/html;charset=UTF-8", captivePortal_html, captivePortal_html_len);
  response->addHeader("Content-Encoding", "gzip");
  response->addHeader("Access-Control-Allow-Origin", "WM_HTTP_CORS_ALLOW_ALL");
  request->send(response);
